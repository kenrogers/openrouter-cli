...
func initListCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Long:    "List all workspaces for the authenticated user. [Management key](/docs/guides/overview/auth/management-api-keys) required.",
		Example: "  openrouter workspaces list",
		RunE:    runListCmd,
	}
	flagutil.RegisterFlags(cmd, listCmdMeta)
	if err := flagutil.ValidateMeta[operations.ListWorkspacesRequest](listCmdMeta); err != nil {
...
func runListCmd(cmd *cobra.Command, args []string) error {
	...
	req, err := flagutil.BuildRequest[operations.ListWorkspacesRequest](cmd, listCmdMeta, "", "")
	if err != nil {
		return err
	}
	...
	if allPages, _ := flagutil.GetBoolFlag(cmd, "all"); allPages && !client.IsDryRun(cmd) {
		...
		res, err := s.Workspaces.List(cmd.Context(), req, sdkOpts...)
		if err != nil {
			return output.Error(cmd, err)
		}
		return output.PaginatedResult(cmd, res, "ListWorkspacesResponse", "Data", maxPages)
	}
