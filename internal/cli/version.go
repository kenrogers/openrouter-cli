...
// which propagates the value here (see cmd/openrouter/main.go):
//
//	go build -ldflags "-X main.version=x.y.z" ./cmd/openrouter
var Version = "0.1.0"
...
func initVersionCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Long: `Print the current version of the openrouter CLI.
...
overridden at build time using Go linker flags:
  ...
  go build -ldflags "-X main.version=x.y.z -X main.buildTime=$(date -u +%Y-%m-%dT%H:%M:%SZ)" ./cmd/openrouter`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if _, err := fmt.Fprintf(cmd.OutOrStdout(), "openrouter %s\n", Version); err != nil {
				return err
			}
			if BuildTime != "" {
