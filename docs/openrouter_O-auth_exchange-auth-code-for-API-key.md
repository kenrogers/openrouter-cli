## openrouter O-auth exchange-auth-code-for-API-key
...
```
openrouter O-auth exchange-auth-code-for-API-key [flags]
```

### Examples

```
  openrouter O-auth exchange-auth-code-for-API-key --code auth_code_abc123def456
```
...
```
      ...
      --code string                    The authorization code received from the OAuth redirect [required]
      --code-challenge-method string   The method used to generate the code challenge (options: S256, plain)
      --code-verifier string           The code verifier if code_challenge was used in the authorization request
  -h, --help                           help for exchange-auth-code-for-API-key
```
