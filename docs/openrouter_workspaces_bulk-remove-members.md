## openrouter workspaces bulk-remove-members
...
```
openrouter workspaces bulk-remove-members [flags]
```

### Examples

```
  openrouter workspaces bulk-remove-members --id production --user-ids '["user_abc123","user_def456"]'
```
...
```
  ...
  -h, --help                   help for bulk-remove-members
  -i, --id string              The workspace ID (UUID) or slug [required]
  -u, --user-ids stringArray   List of user IDs to remove from the workspace [required]
```
