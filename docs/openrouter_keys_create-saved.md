...
Create an OpenRouter API key and save it securely

### Synopsis

Create an OpenRouter API key using the current authenticated key and
...
```
openrouter keys create-saved [flags]
```

### Options

```
  -h, --help                    help for create-saved
      --include-byok-in-limit   Include BYOK usage in the spending limit
      --limit float             Optional spending limit for the API key in USD
      --limit-reset string      Optional reset interval: daily, weekly, or monthly
  -n, --name string             Name for the new API key
