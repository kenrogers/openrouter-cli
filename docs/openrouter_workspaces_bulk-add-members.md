## openrouter workspaces bulk-add-members
...
```
openrouter workspaces bulk-add-members [flags]
```

### Examples

```
  openrouter workspaces bulk-add-members --id production --user-ids '["user_abc123","user_def456"]'
```
...
```
  ...
  -h, --help                   help for bulk-add-members
  -i, --id string              The workspace ID (UUID) or slug [required]
  -u, --user-ids stringArray   List of user IDs to add to the workspace. Members are assigned the same role they hold in the organization. [required]
```
