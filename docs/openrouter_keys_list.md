...
```
  ...
  -i, --include-disabled      Whether to include disabled API keys in the response
      --offset string         Number of API keys to skip for pagination
  -w, --workspace-id string   Filter API keys by workspace ID. By default, keys in the default workspace are returned.
```
