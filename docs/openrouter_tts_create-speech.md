...
```
openrouter tts create-speech [flags]
```

### Examples

```
  openrouter tts create-speech --input Hello world --model elevenlabs/eleven-turbo-v2 --voice alloy
```
...
```
  ...
  -h, --help                     help for create-speech
  -i, --input string             Text to synthesize [required]
  -m, --model string             TTS model identifier [required]
  ...
  -p, --provider string          Provider-specific passthrough configuration
  -r, --response-format string   Audio output format (options: mp3, pcm) (default "pcm")
  -s, --speed float              Playback speed multiplier. Only used by models that support it (e.g. OpenAI TTS). Ignored by other providers.
  -v, --voice string             Voice identifier (provider-specific). [required]
```
