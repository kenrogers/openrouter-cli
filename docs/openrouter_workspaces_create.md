...
```
openrouter workspaces create [flags]
```

### Examples

```
  openrouter workspaces create --name Production --slug production
```
...
```
  ...
  -n, --name string                           Name for the new workspace [required]
  -s, --slug string                           URL-friendly slug (lowercase alphanumeric segments separated by single hyphens, no leading/trailing hyphens) [required]
```
