## openrouter presets create-presets-chat-completions
...
```
openrouter presets create-presets-chat-completions [flags]
```

### Examples

```
  openrouter presets create-presets-chat-completions --slug my-preset --messages '[{"content":"You are a helpful assistant.","role":"system"},{"content":"Hello!","role":"user"}]'
```
...
```
  ...
  -h, --help                                     help for create-presets-chat-completions
  -i, --image-config string                      Provider-specific image configuration options. Keys and values vary by model/provider. See https://openrouter.ai/docs/guides/overview/multimodal/image-generation for more details.
      ...
      --slug string                              URL-safe slug identifying the preset. Created if it does not exist. [required]
