## openrouter O-auth create-auth-code
...
```
openrouter O-auth create-auth-code [flags]
```

### Examples

```
  openrouter O-auth create-auth-code --callback-url https://myapp.com/auth/callback
```
...
```
      ...
      --callback-url string            The callback URL to redirect to after authorization. Note, only https URLs on ports 443 and 3000 are allowed. [required]
      --code-challenge string          PKCE code challenge for enhanced security
      --code-challenge-method string   The method used to generate the code challenge (options: S256, plain)
  -e, --expires-at string              Optional expiration time for the API key to be created
  -h, --help                           help for create-auth-code
  -k, --key-label string               Optional custom label for the API key. Defaults to the app name if not provided.
  -l, --limit float                    Credit limit for the API key to be created
  -u, --usage-limit-type string        Optional credit limit reset interval. When set, the credit limit resets on this interval. (options: daily, weekly, monthly)
```
